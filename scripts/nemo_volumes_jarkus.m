function [V,index,t_index]=nemo_volumes_jarkus(D,altitude,index,t_index,reach)
...
switch reach
    case 'all'
        ...
        %d=1623.*ones(size(D.alongshore)); %=6000m from RSP
...
a=sum(~isnan(D.altitude),3);
t_mask=a>20;
t_idx=1:52;
%% Calculate Volumes
for i = index;%1:length(trans2run)
    t_index=t_idx(t_mask(:,i));
...
for i = index; %1:length(trans2run)
    ...
    for t= 1:length(t_index)-1;
        V.dt(t,i)=D.time_bathy(t_index(t+1))-D.time_bathy(t_index(t));
        mask=D.dist>=V.dist_on(i) & D.dist<=V.dist_off(i);
        if sum(~isnan(squeeze(z(t_index(t),i,mask))) & ~isnan(squeeze(z(t_index(t+1),i,mask))))>10 %30
        %if sum(~isnan(squeeze(z(t,i,:))) & ~isnan(squeeze(z(t_index(t_idx+1),i,:))))>10 %30
            [V.dv(t_index(t),i), temp] = jarkus_getVolume(D.dist, squeeze(z(t_index(t),i,:)),10, -15, V.dist_on(i), V.dist_off(i), D.dist,squeeze(z(t_index(t+1),i,:)));
           ...
           %[net_volume,struct]=jarkus_getVolume(cross-points_1,altitude_1,max_alti,min_alti,onsh-bound,offsh_bound,cross-points_2,altitude_2);
            V.accretion(t_index(t),i) = temp.Volumes.Accretion;
            V.erosion(t_index(t),i)   = temp.Volumes.Erosion;
        %else
            %dV(t,i)=nan;
        end
    end
