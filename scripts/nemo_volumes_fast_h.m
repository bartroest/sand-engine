function [VH,index,t_index]=nemo_volumes_fast_h(D,altitude,index,t_index,heights)%,reach)
%NEMO_VOLUMES_H Calculates volume changes of transects in horizontal slices.
%
...
%         
% Example:
% [VH, idx, t_idx]=nemo_volumes(D,'altitude',[1:10:length(D.alongshore)],[1,7,37],[-12:1:5]);
%
...
for t= t_index(1:end-1);
    ...
    VH.dt(t)=D.time(t_index(t_idx+1))-D.time(t);
    for i = index;
        if sum(~isnan(squeeze(z(t,i,:))) & ~isnan(squeeze(z(t_index(t_idx+1),i,:))))>2
            for h = 1:length(heights)-1;
                VH.dv(t,i,h)=jarkus_getVolume(D.dist, squeeze(z(t,i,:)), heights(h+1), heights(h), -500, 3000, D.dist, squeeze(z(t_index(t_idx+1),i,:))); 
            end
        end
    end
end
